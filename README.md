...
Two Claude Code skills for doing quantitative analysis work in Stata: `stata-data-analysis` and
`stata-data-cleaning`. See each folder's `SKILL.md` for what it actually does. (This repo itself
...
root of a clone. If you clone to one location and then move `stata-data-analysis/` and
`stata-data-cleaning/` out into a *different* folder, those moved folders are just plain files
...
or an earlier non-git copy of these same two), `git clone` will refuse to run — move anything
...
`https://github.com/dtorregibney/claude-skills.git`.
...
1. Copy `stata-data-analysis/` and `stata-data-cleaning/` out of your skills folder to a backup
   location first — don't skip this.
2. Remove those same two folders from your skills folder (clearing the way for a clean clone).
